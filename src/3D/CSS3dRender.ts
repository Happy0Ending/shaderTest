...
class CSS3DRenderer {
    ...
    render(HTMLMesh: CSS3DObject, camera: Camera) {
        ...
        this.renderObject(HTMLMesh, HTMLMesh.getScene(), camera, cameraCSSMatrix)
    }
}

class CSS3DObject extends Mesh {
    public element: HTMLElement;
    public topParent: HTMLElement | undefined;
    constructor(element: HTMLElement, name: string, scene: Scene) {
 ...
 */
export function createHtmlMesh(node: Mesh, canvas: HTMLCanvasElement, src: HTMLElement) {
    let renderer = setupRenderer(canvas);
    const mesh = createCSSobject(node as Mesh, node.getScene(), src);
    mesh.topParent = renderer.domElement.parentElement as HTMLElement;
    node.getScene().onBeforeRenderObservable.add(() => {
        renderer.render(mesh, node.getScene().activeCamera as Camera)
    })
