import { ArcRotateCamera, BackgroundMaterial, Color3, Color4, CubeTexture, Engine, GlowLayer, HemisphericLight, Mesh, MeshBuilder, MirrorTexture, PBRMaterial, PBRMetallicRoughnessMaterial, Plane, RenderTargetTexture, Scene, SceneLoader, Size, StandardMaterial, StringTools, Texture, Vector2, Vector3 } from "babylonjs";
import { AdvancedDynamicTexture, ColorPicker, Rectangle, Slider, TextBlock } from "babylonjs-gui";
import { WaterMaterial } from "babylonjs-materials";

export function textureScene(engine: Engine, canvas: HTMLCanvasElement) {
    ...
    SceneLoader.ImportMesh("", '', "red.glb", scene, (meshes) => {
        ...
        meshes.forEach((mesh)=>{
            console.log("texture",mesh.material?.getActiveTextures());
            mesh.material?.getActiveTextures().forEach((texture)=>{
                if(texture.getInternalTexture()?._buffer){
                    const dom = document.createElement("img");
                    let a =  StringTools.EncodeArrayBufferToBase64(texture.getInternalTexture()?._buffer as ArrayBuffer)
                    console.log(a);
